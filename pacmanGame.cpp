...
static int vetorPercursoX[1000];
static int vetorPercursoY[1000];
...
static int cv_X = 1000;
static int cv_Y = 580;
static int delay = 40;

static int cv2_X = 1440;
static int cv2_Y = 580;

static int vetorPercursoX2[1000];
static int vetorPercursoY2[1000];

static int indiceM2 = 0;
static int indiceE2 = 0;
...
// static int percursoVilaoX[12] = {cv_X, cv_X - 40, cv_X - 80, cv_X - 120, cv_X - 160, cv_X - 320};
// static int percursoVilaoY[12] = {cv_Y, cv_Y, cv_Y, cv_Y, cv_Y, cv_Y - 300};
...
//   glClear(GL_COLOR_BUFFER_BIT);
    ...
    // glColor4f(1.0, 1.0, 1.0, 0.5);
    // glRecti(vetorPercursoX2[indiceM2], vetorPercursoY2[indiceM2], vetorPercursoX2[indiceM2]+60, vetorPercursoY2[indiceM2]+60);
    ...
    glBegin(GL_QUADS);
    glTexCoord3f(0, 0, 0);
    glVertex3f(vetorPercursoX[indiceM], vetorPercursoY[indiceM], 0);
    glTexCoord3f(0, 1, 0);
    glVertex3f(vetorPercursoX[indiceM], vetorPercursoY[indiceM] + 60, 0);
    glTexCoord3f(1, 1, 0);
    glVertex3f(vetorPercursoX[indiceM] + 60, vetorPercursoY[indiceM] + 60, 0);
    glTexCoord3f(1, 0, 0);
    glVertex3f(vetorPercursoX[indiceM] + 60, vetorPercursoY[indiceM], 0);
    glEnd(); // angulo += velocidade;
    glDisable(GL_TEXTURE_2D);
    ...
    glBegin(GL_QUADS);
    glTexCoord3f(0, 0, 0);
    glVertex3f(vetorPercursoX2[indiceM2], vetorPercursoY2[indiceM2], 0);
    glTexCoord3f(0, 1, 0);
    glVertex3f(vetorPercursoX2[indiceM2], vetorPercursoY2[indiceM2] + 60, 0);
    glTexCoord3f(1, 1, 0);
    glVertex3f(vetorPercursoX2[indiceM2] + 60, vetorPercursoY2[indiceM2] + 60, 0);
    glTexCoord3f(1, 0, 0);
    glVertex3f(vetorPercursoX2[indiceM2] + 60, vetorPercursoY2[indiceM2], 0);
    glEnd(); // angulo += velocidade;
    glDisable(GL_TEXTURE_2D);    
...
// commit/c79b2fc819a1ef5feb331db1607a0f88c36a042a
void keyboard(unsigned char key, int x, int y)
{
    switch (key)
    {
...
void carregarPercurso(){
    ...
    vetorPercursoX[0] = posicaoX;
    vetorPercursoY[0] = posicaoY;

    int i = 1;

    while (posicaoX >= 470){
        posicaoX -= 10;
        vetorPercursoY[i] = posicaoY;
        vetorPercursoX[i] = posicaoX;
        i++;
    }
    ...
    vetorPercursoX2[0] = posX;
    vetorPercursoY2[0] = posY;

    int j = 1;

    while (posX >= 1240){
        posX -= 10;
        vetorPercursoY2[j] = posY;
        vetorPercursoX2[j] = posX;
        j++;
    }
...
bool colideVilao(int posicaoX, int posicaoY)
{
    ...
    int auxiliarCoordX = vetorPercursoX[indiceM];
    int auxiliarCoordY = vetorPercursoY[indiceM];
    bool colisao = false;
